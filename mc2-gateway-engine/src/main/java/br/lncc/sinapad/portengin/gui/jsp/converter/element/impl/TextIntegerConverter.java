...
public class TextIntegerConverter implements ElementConverter<String, TextInteger> {

	public String convert(TextInteger e, Properties locale) {
		String str = "<div class=\"alert alert-danger alert-dismissable\" id=\"" + e.getName() + "_err\" style=\"display:none\">" + "<button type=\"button\" id=\"" + e.getName() + "_close\" class=\"close\" >&times;</button>" + "<strong><s:text name=\"class.TextInteger.minmax.msn\" /> (min: " + (e.getMin() != null ? e.getMin() : "unbound") + " max: " + (e.getMax() != null ? e.getMax() : "unbound") + ")</strong></div><script type=\"text/javascript\">$('#" + e.getName() + "_close').on('click', function () {$('#" + e.getName() + "_err').hide();});</script>" + "<div id=\"" + e.getName() + "_tip\" style=\"border-style: dotted; border-color: black; display: none;\"><h6>" + e.getTip(locale) + "</h6></div><div class=\"form-group\" "+getHiddenEvent(e)+" id=\"" + e.getName() + "_form\"><label for=\"" + e.getName() + "\" id=\"" + e.getName() + "_label\">" + e.getLabel(locale);
