...
public class ManagePublicFilesAction extends BaseAction {

	private static Logger logger = Logger.getLogger(ManagePublicFilesAction.class);
	...
	@Action(value = "/managePublicFiles", results = { @Result(name = "success", location = "/public_file_manager.jsp"), @Result(name = "error", location = "/public_file_manager.jsp") })
	public String managePublicFiles() {
		...
		try {
			if (!isGuest) {
				...
				if (fileService.exists(publicUuid, PortEnginConstants.PORTENGIN_PUBLIC_PROJECT, parentsArr, fileName)) {
					FileData data = fileService.find(publicUuid, PortEnginConstants.PORTENGIN_PUBLIC_PROJECT, parentsArr, fileName);
